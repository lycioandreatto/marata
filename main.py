...
if df_base is not None and not df_base.empty:
    ...
    col_base_nome = pick_col(df_base, ["Nome 1", "NOME 1", "Nome", "NOME", "Razão Social", "RAZAO SOCIAL"], fallback=None)
    ...
    if col_fat_cliente_cod is not None:
        set_fat = set([str(x) for x in df_fat[col_fat_cliente_cod].dropna().unique().tolist() if str(x).strip() != ""])
    else:
        set_fat = set()
    ...
    if alerta_sel is not None:
        ...
        if alerta_sel == "SEM_FAT":
            ...
            # ✅ lista com NOME + CÓDIGO (se existir "Nome 1" na BASE)
            if sem_faturamento:

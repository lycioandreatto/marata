...
# --- PÁGINA: DESEMPENHO DE VENDAS (FATURADO) ---
    ...
    try:
        ...
        if df_param_metas is not None:
            ...
            # Mapeamos a PARAM_METAS para olhar por ESTADO (Ajuste conforme o nome da coluna de estado na sua aba de parâmetros)
            if 'ESTADO' not in df_param_metas.columns and 'ESCRV' in df_param_metas.columns:
    ...
    if df_faturado is not None and not df_faturado.empty:
        ...
        # --- LÓGICA DE POSITIVAÇÃO E METAS (SÓ ESTADO / SUP / VEND) ---
        if not df_f.empty:
            # Regra: Se for visão de Estado ou Analista, ignora STR e SMX
            if not (sel_supervisor or sel_vendedor):
                ...
                # Meta buscada pelo ESTADO (ignora o analista no cálculo)
                estado_ref = sel_estado[0] if sel_estado else df_f['ESTADO'].iloc[0]
                linha_meta = df_param_metas[df_param_metas['ESTADO'] == estado_ref]
            ...
            with m3:
                ...
                <div style="border: 1px solid #ddd; padding: 15px; border-radius: 8px; background-color: #f9f9f9;">
                    <small style="color: #666;">COBERTURA (VISÃO: {"VENDEDOR/SUP" if (sel_supervisor or sel_vendedor) else "ESTADO"})</small><br>
        ...
        st.dataframe(df_final_h.style.format({'Volume': '{:,.0f}', 'Positivação': '{:,.0f}'}), use_container_width=True, hide_index=True)

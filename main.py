...
if menu == "Novo Agendamento":
    ...
    if eh_admin:
        lista_sups = sorted([s for s in df_base['Região de vendas'].unique() if str(s) != 'nan'])
    ...
    if sup_alvo != "Selecione...":
        ...
        if not clientes_f.empty:
            # Pega o nome do analista da coluna ANALISTA (Coluna A da BASE)
                ...
                with st.form("f_novo"):
                    ...
                    if st.form_submit_button("💾 SALVAR"):
                        ...
                        nova_linha = pd.DataFrame([{
                            ...
                            "ANALISTA": nome_analista, # Salva na Coluna D da AGENDA
                            "SUPERVISOR": sup_alvo,
...
# 2. MINHA AGENDA
    ...
    if not eh_admin:
        ...
        f_sup = st.selectbox("Filtrar Supervisor:", ["Todos"] + sorted(df_agenda['SUPERVISOR'].unique().tolist()))
        if f_sup != "Todos": df_f = df_f[df_f['SUPERVISOR'] == f_sup]

    if not df_f.empty:
        df_f["EDITAR"] = False
        # Exibição na tabela (incluindo Analista para conferência)
        ...
        if not marcadas.empty:
            ...
            with st.form("edit_marata"):
                ...
                obs_extra = ""
                if n_st == "OUTRO" or n_ju == "OUTRO":
                    obs_extra = st.text_input("Descreva o motivo (Opção Outro):")

                if st.form_submit_button("✅ ATUALIZAR"):
                    # Se escreveu algo no campo "Outro", substitui o valor

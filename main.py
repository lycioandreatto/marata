...
# --- PÁGINA: NOVO AGENDAMENTO ---
    ...
    if df_base is not None:
        # Identificação de colunas na Base para flexibilidade
        ...
        with f1:
            if is_admin or is_diretoria:
                lista_ana = sorted([str(a) for a in df_base[col_ana_base].unique() if str(a).strip() and str(a).lower() != 'nan'])
        ...
        with f2:
            ...
            lista_sups = sorted([str(s) for s in df_sups_f[col_rv_base].unique() if str(s).strip() and str(s).lower() != 'nan'])
            sup_sel = st.selectbox("Selecione o Supervisor:", lista_sups)

        # 2. Listagem de Clientes para Seleção
        st.markdown(f"### Clientes de {sup_sel}")
        df_clientes = df_base[df_base[col_rv_base] == sup_sel].copy()
        df_clientes['Selecionar'] = False
        
        # Reordenar para mostrar checkbox primeiro
        cols_mostrar = ['Selecionar', 'Cliente', 'Nome', col_local]
        ...
        grid_clientes = st.data_editor(
            ...
            column_config={
                ...
                "Cliente": st.column_config.TextColumn("Código", disabled=True)
            },
            key="grid_novo_ag"
        )

        # 3. Data e Finalização
        data_visita = st.date_input("Data da Visita:", datetime.now(fuso_br))
        
        if st.button("🚀 FINALIZAR AGENDAMENTO EM MASSA"):
            ...
            if not selecionados.empty:
                ...
                for _, row in selecionados.iterrows():
                    ...
                    registro_hora = datetime.now(fuso_br).strftime("%d/%m/%Y %H:%M")
                    
                    novos_registros.append({
                        "ID": novo_id,
                        "DATA": data_visita.strftime("%d/%m/%Y"),
                        "ANALISTA": ana_sel if ana_sel != "Todos" else user_atual,
                        "SUPERVISOR": sup_sel,
                        "CÓDIGO CLIENTE": str(row['Cliente']),
                        "CLIENTE": row['Nome'],
                        "STATUS": "Planejado",
                        "JUSTIFICATIVA": "-",
                ...
                df_agenda_total = pd.concat([df_agenda, df_novos], ignore_index=True)
                
                # Salva na Planilha Google
                conn.update(spreadsheet=url_planilha, worksheet="AGENDA", data=df_agenda_total.drop(columns=['LINHA'], errors='ignore'))
                
                st.success(f"✅ Sucesso! {len(selecionados)} visitas agendadas para {data_visita.strftime('%d/%m/%Y')}.")
                st.cache_data.clear()
                time.sleep(1)
                st.rerun()
            else:
                st.error("Selecione pelo menos um cliente na tabela acima.")
    else:

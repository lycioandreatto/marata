...
# --- PÁGINA: SIMULADOR DE METAS (SIMULADO) ---
elif menu == "📈 Simulador de Metas (SIM)":

    import numpy as np
    import pandas as pd
    from datetime import datetime

    st.header("📈 Maratá — Simulador de Metas (Dados Simulados)")
    st.caption("Comparativo: Meta Atual (Ano anterior + %) vs Meta Nova (Capacidade/Potencial real). Tudo simulado para demonstração.")

    # =========================================================
    # 1) Produtos (os seus)
    # =========================================================
    PRODUTOS = [
        "ACHOCOLATADO","ACUCAR","ADOCANTE SACARINA","ADOCANTE SUCRALOSE","AZEITONA","BALSAMICO",
        "BEBIDA MISTA","CALDOS TABLETE","CATCHUP","CEBOLINHA","COGUMELO","DESCARTAVEIS","ESPECIARIAS",
        "FARINHA DE TRIGO FD","FARINHA DE TRIGO SC","FARINHA LACTEA","FERMENTO","MACARRAO INSTANTANEO",
        "MARATINHO","MILHO","MILHO FARINHA GOTA","MILHO FARINHA MARATA","MILHO FLOCAO GOTA","MILHO FLOCAO MARATA",
        "MILHO PIPOCA","MINGAU","MISTURA BOLO","MOLHO PRONTO","MOLHOS ALHO","MOLHOS INGLES","MOLHOS LIMAO",
        "MOLHOS PIMENTA","MOLHOS PIMENTA 75ML","MOLHOS SALSA","MOLHOS SHOYO","MOLHOS TEMPEROS CASEIROS",
        "OLEAGINOSAS","PIMENTA CONSERVA","REFRESCO","SALGADINHOS FARDO","SALGADINHOS NACHOS","SALGADINHOS PASTEIS",
        "SUCO D+ 1000ML","SUCO D+ 200ML","SUCO MARATA 1000ML","SUCO MARATA 200ML","TEMPERO COLORIFICO GOTA",
        "TEMPERO COLORIFICO MARATA","TEMPERO CONDIMENTO GOTA","TEMPERO CONDIMENTO MARATA","TEMPERO EM PO","VINAGRE","VINAGRE ESPECIAL"
    ]
    ...
    with c1:
        n_vendedores = st.slider("Qtd vendedores", 5, 30, 12)
    with c2:
        meses_passado = st.slider("Meses no passado", 6, 24, 12)
    with c3:
        meses_futuro = st.slider("Meses no futuro", 3, 12, 6)
    with c4:
        semente = st.number_input("Semente (repetir simulação)", min_value=1, max_value=999999, value=2026, step=1)
    ...
    with m2:
        fechamento_gap = st.slider("Meta Nova: fecha % do GAP por mês", 5, 35, 15) / 100.0
    with m3:
        pressao_mercado = st.slider("Dificuldade do mês (simula cenário ruim)", 0, 30, 12) / 100.0

    st.info(
        "📌 **Como a simulação funciona (simples e realista):**\n"
        "- Cada vendedor tem um **baseline** por produto (capacidade atual)\n"
        "- Existe um **potencial** por produto (capacidade máxima provável)\n"
        "- A Meta Atual usa **ano anterior + %**\n"
        "- A Meta Nova usa **baseline + crescimento gradual do gap**\n"
        "- E aplicamos uma “dificuldade” para simular que **historicamente ninguém bate a meta atual**."
    )

    # =========================================================
    # 3) Gerador de dados simulados
    # =========================================================
    @st.cache_data(ttl=300)
    def gerar_dados_simulados(seed, n_vend, meses_hist, meses_fore, dificuldade):
        rng = np.random.default_rng(int(seed))

        # vendedores
        vendedores = [f"VEND_{i:02d}" for i in range(1, n_vend + 1)]

        # mês atual (Period M)
        hoje_p = pd.Timestamp(datetime.now().date()).to_period("M")

        # meses históricos (inclui mês atual)
        meses_all_p = [hoje_p - i for i in range(meses_hist, 0, -1)] + [hoje_p]
        meses_all = [str(p) for p in meses_all_p]

        # meses futuros
        fut_p = [hoje_p + i for i in range(1, meses_fore + 1)]
        fut = [str(p) for p in fut_p]

        # “peso” do produto (alguns naturalmente maiores)
        peso_prod = {}
        for p in PRODUTOS:
            base = rng.uniform(0.9, 3.2)
            if "MILHO" in p or "FARINHA" in p or "SUCO" in p or "REFRESCO" in p:
                base *= rng.uniform(1.2, 1.7)
            peso_prod[p] = float(min(base, 4.0))

        # baseline por vendedor x produto (capacidade atual) — já como INTEIRO realista
        baseline = []
        for v in vendedores:
            fator_v = rng.uniform(0.75, 1.25)
            for p in PRODUTOS:
                # aqui define escala realista (centenas a poucos milhares)
                b = rng.uniform(180, 900) * peso_prod[p] * fator_v

                # controla extremos (não deixa ficar 20k do nada)
                b = float(np.clip(b, 80, 9000))

                baseline.append((v, p, int(round(b))))
        df_base = pd.DataFrame(baseline, columns=["VENDEDOR", "PRODUTO", "BASELINE"])

        # potencial (capacidade) >= baseline
        mult_pot = rng.uniform(1.15, 1.80, size=len(df_base))
        df_base["POTENCIAL"] = (df_base["BASELINE"].astype(float) * mult_pot).round().astype(int)

        # histórico de vendas (inteiro), com ruído e sazonalidade leve
        linhas_hist = []
        for idx_mes, ym in enumerate(meses_all):
            saz = rng.normal(0.0, 0.06)  # 6%
            for _, r in df_base.iterrows():
                v = r["VENDEDOR"]
                p = r["PRODUTO"]
                b = float(r["BASELINE"])

                ruido = rng.normal(0.0, 0.18)  # 18%
                venda = b * (1.0 + saz + ruido) * (1.0 - dificuldade)

                # deixa as vendas “mais críveis” (inteiro)
                venda = float(np.clip(venda, 0, None))
                venda_int = int(round(venda))

                linhas_hist.append((ym, v, p, venda_int))

        df_hist = pd.DataFrame(linhas_hist, columns=["YM", "VENDEDOR", "PRODUTO", "VENDAS"])
        df_hist_agg = df_hist.groupby(["YM", "VENDEDOR", "PRODUTO"], as_index=False)["VENDAS"].sum()

        return df_base, df_hist_agg, meses_all, fut, str(hoje_p)

    df_base, df_hist, meses_hist_list, meses_fut_list, mes_atual = gerar_dados_simulados(
        semente, n_vendedores, meses_passado, meses_futuro, pressao_mercado
    )

    # =========================================================
    # 4) Cálculo de metas (mês atual)
    # =========================================================
    mes_atual_p = pd.Period(mes_atual, freq="M")
    aa_mes_atual = str(mes_atual_p - 12)

    df_mes_atual = df_hist[df_hist["YM"] == mes_atual].copy()
    df_aa_mes = df_hist[df_hist["YM"] == aa_mes_atual].copy()
    df_aa_mes = df_aa_mes.rename(columns={"VENDAS": "VENDAS_AA_MES"})

    df_comp = df_mes_atual.merge(
        df_aa_mes[["VENDEDOR", "PRODUTO", "VENDAS_AA_MES"]],
        on=["VENDEDOR", "PRODUTO"],
        how="left"
    )
    df_comp["VENDAS_AA_MES"] = df_comp["VENDAS_AA_MES"].fillna(0).astype(int)

    df_comp = df_comp.merge(
        df_base[["VENDEDOR", "PRODUTO", "BASELINE", "POTENCIAL"]],
        on=["VENDEDOR", "PRODUTO"],
        how="left"
    )

    df_comp["META_ATUAL"] = (df_comp["VENDAS_AA_MES"].astype(float) * (1.0 + pct_meta_antiga)).round().astype(int)

    df_comp["GAP"] = (df_comp["POTENCIAL"].astype(float) - df_comp["BASELINE"].astype(float)).clip(lower=0.0)
    df_comp["META_NOVA"] = (df_comp["BASELINE"].astype(float) + df_comp["GAP"] * fechamento_gap).round().astype(int)

    df_comp["ATING_%_ATUAL"] = np.where(
        df_comp["META_ATUAL"] > 0,
        df_comp["VENDAS"].astype(float) / df_comp["META_ATUAL"].astype(float) * 100.0,
        0.0
    )
    df_comp["ATING_%_NOVA"] = np.where(
        df_comp["META_NOVA"] > 0,
        df_comp["VENDAS"].astype(float) / df_comp["META_NOVA"].astype(float) * 100.0,
        0.0
    )

    # =========================================================
    # 5) Filtros do dashboard
    # =========================================================
    st.markdown("### 🔎 Análise")
    f1, f2, f3 = st.columns([0.4, 0.35, 0.25])

    with f1:
        vend_sel = st.selectbox("Vendedor", ["(Todos)"] + sorted(df_comp["VENDEDOR"].unique().tolist()), index=0)
    with f2:
        prod_sel = st.selectbox("Produto", ["(Todos)"] + PRODUTOS, index=0)
    with f3:
        visao = st.radio("Visão", ["Receita/Volume (simulado)", "Atingimento (%)"], horizontal=True)

    df_view = df_comp.copy()
    if vend_sel != "(Todos)":
        df_view = df_view[df_view["VENDEDOR"] == vend_sel]
    if prod_sel != "(Todos)":
        df_view = df_view[df_view["PRODUTO"] == prod_sel]

    # =========================================================
    # 6) KPIs (mês atual)
    # =========================================================
    total_vendas = int(df_view["VENDAS"].sum())
    total_meta_atual = int(df_view["META_ATUAL"].sum())
    total_meta_nova = int(df_view["META_NOVA"].sum())

    at_atual = (total_vendas / total_meta_atual * 100.0) if total_meta_atual > 0 else 0.0
    at_nova = (total_vendas / total_meta_nova * 100.0) if total_meta_nova > 0 else 0.0

    k1, k2, k3, k4 = st.columns(4)
    k1.metric("Mês atual (simulado)", mes_atual)
    k2.metric("Vendas (simulado)", f"{total_vendas:,.0f}".replace(",", "X").replace(".", ",").replace("X", "."))
    k3.metric("% Atingimento Meta Atual", f"{at_atual:.1f}%")
    k4.metric("% Atingimento Meta Nova", f"{at_nova:.1f}%")

    st.markdown("---")

    # =========================================================
    # 7) Tabela comparativa (top gaps)
    # =========================================================
    st.markdown("### 📋 Onde está o problema (mês atual)")
    tab = df_view.copy()
    tab["GAP_META_ATUAL"] = tab["META_ATUAL"] - tab["VENDAS"]
    tab["GAP_META_NOVA"] = tab["META_NOVA"] - tab["VENDAS"]

    tab_show = tab[
        ["VENDEDOR", "PRODUTO", "VENDAS", "META_ATUAL", "META_NOVA", "ATING_%_ATUAL", "ATING_%_NOVA", "GAP_META_ATUAL", "GAP_META_NOVA"]
    ].copy()

    tab_show = tab_show.sort_values("GAP_META_ATUAL", ascending=False).head(50)
    st.dataframe(tab_show, use_container_width=True, hide_index=True)

    st.markdown("---")

    # =========================================================
    # 8) Série temporal: passado + futuro
    #    - Meta Nova cresce (sempre) no futuro
    #    - Vendas ficam inteiras
    # =========================================================
    st.markdown("### 📈 Passado x Futuro (simulado)")
    st.caption("No futuro, a Meta Nova cresce fechando o GAP mês a mês (linha sempre sobe).")

    if vend_sel == "(Todos)":
        vend_plot = st.selectbox("Escolha um vendedor para o gráfico", sorted(df_comp["VENDEDOR"].unique().tolist()))
    else:
        vend_plot = vend_sel

    if prod_sel == "(Todos)":
        prod_plot = st.selectbox("Escolha um produto para o gráfico", PRODUTOS)
    else:
        prod_plot = prod_sel

    # histórico do par
    h = df_hist[(df_hist["VENDEDOR"] == vend_plot) & (df_hist["PRODUTO"] == prod_plot)].copy()
    h["_P"] = pd.PeriodIndex(h["YM"], freq="M")
    h = h.sort_values("_P")

    # base e potencial
    bp = df_base[(df_base["VENDEDOR"] == vend_plot) & (df_base["PRODUTO"] == prod_plot)].copy()
    baseline = int(bp["BASELINE"].iloc[0]) if not bp.empty else 0
    potencial = int(bp["POTENCIAL"].iloc[0]) if not bp.empty else baseline
    gap_total = max(0, potencial - baseline)

    # ========= META ATUAL no histórico =========
    # Para cada mês M: META_ATUAL(M) = VENDAS(M-12) * (1+%)
    h_aa = h.copy()
    h_aa["_P"] = h_aa["_P"] + 12  # desloca AA para alinhar no mês M
    h_aa = h_aa.rename(columns={"VENDAS": "VENDAS_AA"})

    h_merge = h.merge(h_aa[["_P", "VENDAS_AA"]], on="_P", how="left")
    h_merge["VENDAS_AA"] = h_merge["VENDAS_AA"].fillna(0).astype(int)
    h_merge["META_ATUAL"] = (h_merge["VENDAS_AA"].astype(float) * (1.0 + pct_meta_antiga)).round().astype(int)

    # ========= META NOVA no histórico =========
    # evolução de capacidade: baseline + (gap * fechamento_gap_acumulado)
    # vamos simular uma trajetória crescente “de aprendizado” no passado também
    # (apenas para ficar apresentável)
    h_merge = h_merge.sort_values("_P").reset_index(drop=True)

    meta_nova_hist = []
    acumulado_gap = 0.0
    for _ in range(len(h_merge)):
        acumulado_gap = min(float(gap_total), acumulado_gap + float(gap_total) * float(fechamento_gap))
        meta_nova_val = float(baseline) + acumulado_gap
        meta_nova_hist.append(int(round(meta_nova_val)))

    # garante monotonicidade
    meta_nova_hist = np.maximum.accumulate(np.array(meta_nova_hist)).astype(int)
    h_merge["META_NOVA"] = meta_nova_hist

    # ========= FUTURO =========
    fut_periods = [pd.Period(mes_atual, freq="M") + i for i in range(1, meses_futuro + 1)]
    fut_ym = [str(p) for p in fut_periods]

    # Pega a meta atual “base” do mês atual para projeção simples
    meta_atual_mes_atual = int(
        df_comp[(df_comp["VENDEDOR"] == vend_plot) & (df_comp["PRODUTO"] == prod_plot)]["META_ATUAL"].sum()
    )

    # começa do último valor histórico da meta nova e cresce fechando gap até potencial
    ultimo_meta_nova = int(h_merge["META_NOVA"].iloc[-1]) if len(h_merge) else baseline

    linhas_f = []
    meta_nova_atual = float(ultimo_meta_nova)

    for ym in fut_ym:
        # cresce sempre: aproxima do potencial
        falta = max(0.0, float(potencial) - meta_nova_atual)
        incremento = falta * float(fechamento_gap)
        meta_nova_atual = meta_nova_atual + max(1.0, incremento)  # garante que sobe pelo menos 1

        meta_nova_atual = min(float(potencial), meta_nova_atual)

        linhas_f.append({
            "YM": ym,
            "VENDAS": np.nan,
            "META_ATUAL": meta_atual_mes_atual,
            "META_NOVA": int(round(meta_nova_atual)),
        })

    df_plot_hist = h_merge[["YM", "VENDAS", "META_ATUAL", "META_NOVA"]].copy()
    df_plot_fut = pd.DataFrame(linhas_f)

    df_plot = pd.concat([df_plot_hist, df_plot_fut], ignore_index=True)
    df_plot["_P"] = pd.PeriodIndex(df_plot["YM"], freq="M")
    df_plot = df_plot.sort_values("_P").drop(columns=["_P"])

    # garante inteiros no chart onde dá
    df_plot["VENDAS"] = pd.to_numeric(df_plot["VENDAS"], errors="coerce")
    df_plot["META_ATUAL"] = pd.to_numeric(df_plot["META_ATUAL"], errors="coerce").fillna(0).round().astype(int)
    df_plot["META_NOVA"] = pd.to_numeric(df_plot["META_NOVA"], errors="coerce").fillna(0).round().astype(int)

    st.line_chart(df_plot.set_index("YM")[["VENDAS", "META_ATUAL", "META_NOVA"]])

    st.markdown("---")

    # =========================================================
    # 9) Conclusão executiva (pra diretoria)
    # =========================================================
    st.markdown("### 🧩 Leitura executiva (Maratá) — argumentos prontos")

    st.write(
        "**1) Meta Atual (Ano anterior + %) pode ficar impossível** quando o mercado muda, quando há ruptura, "
        "quando a base já está comprimida, ou quando a execução não tem capacidade para absorver esse crescimento."
    )
    st.write(
        "**2) Meta por Capacidade** não “abaixa a régua”: ela **ancora no que é possível** (baseline) e "
        "**empurra crescimento com disciplina** (fechando o GAP para o potencial)."
    )
    st.write(
        "**3) Isso vira plano de ação de verdade:** você enxerga onde o gap é maior por vendedor/produto "
        "e define rotina (mix, cobertura, frequência, negociação, execução) para fechar o gap gradualmente."
    )

    st.success("✅ Página 100% simulada, pronta para apresentar. Se quiser, eu deixo o visual no padrão premium igual seu Menu Início.")
...
# --- PÁGINA: INSIGHTS FATURADO ---
    ...
    with col_titulo:
        st.header("🗺️ INSIGHTS FATURADO")

    with col_btn:
        if st.button("🔄 Atualizar", key="btn_refresh_insights_faturado"):
            st.cache_data.clear()
            st.rerun()
    ...
    def _dedup_columns(cols):
        """
        Se vier coluna duplicada (ex: Cliente 2x), renomeia automaticamente:
        Cliente -> Cliente | Cliente_2 -> Cliente_3 ...
        """
        seen = {}
        out = []
        for c in cols:
    ...
    @st.cache_data(ttl=300)
    def _load_faturado():
        df = conn.read(spreadsheet=url_planilha, worksheet="FATURADO")
        if df is None or df.empty:
            return None

        df = df.dropna(how="all").copy()
        df.columns = _dedup_columns([_safe_str(c) for c in df.columns])

        # Padroniza nomes esperados (se existirem)
        # (mantém o que já vier do Sheets, mas cria aliases úteis)
        col_map = {
            "ANALISTA": "ANALISTA",
            "EscrV": "ESTADO",
            "Data fat.": "DATA_FAT",
        ...
        # Tipos
        if "DATA_FAT" in df.columns:
            df["DATA_FAT"] = _to_datetime(df["DATA_FAT"])
        else:
            df["DATA_FAT"] = pd.NaT
        ...
        for c in ["ANALISTA", "ESTADO", "VENDEDOR", "COD_SUP", "COD_VEND", "CLIENTE_NOME", "CLIENTE_COD", "HIERARQUIA", "SKU_NOME", "SKU_COD", "CIDADE"]:
            if c in df.columns:
                df[c] = df[c].apply(_safe_str)

        # Datas auxiliares
        df["ANO_MES"] = df["DATA_FAT"].dt.strftime("%Y-%m")
        return df

    df_fat = _load_faturado()

    if df_fat is None or df_fat.empty:
        st.warning("A aba FATURADO está vazia ou não foi possível ler. Verifique o nome da aba e as permissões.")
    else:
        # ============================
        # Filtros
        # ============================
        ...
        # Range de datas
        min_dt = df_fat["DATA_FAT"].min()
        max_dt = df_fat["DATA_FAT"].max()

        colf1, colf2, colf3, colf4 = st.columns([0.32, 0.22, 0.22, 0.24])
        ...
        df = df_fat.copy()
        df = df[(df["DATA_FAT"].dt.date >= dt_ini) & (df["DATA_FAT"].dt.date <= dt_fim)].copy()

        # Dropdowns (dependem do que existir)
        estados = sorted([x for x in df["ESTADO"].unique().tolist() if _safe_str(x) != ""]) if "ESTADO" in df.columns else []
        analistas = sorted([x for x in df["ANALISTA"].unique().tolist() if _safe_str(x) != ""]) if "ANALISTA" in df.columns else []
        supers = sorted([x for x in df["COD_SUP"].unique().tolist() if _safe_str(x) != ""]) if "COD_SUP" in df.columns else []
        vendedores = sorted([x for x in df["VENDEDOR"].unique().tolist() if _safe_str(x) != ""]) if "VENDEDOR" in df.columns else []

        with colf3:
            estado_sel = st.multiselect("Estado", options=estados, default=[], key="fat_estado_sel")

        with colf4:
            analista_sel = st.multiselect("Analista", options=analistas, default=[], key="fat_analista_sel")

        colf5, colf6 = st.columns(2)
        with colf5:
            sup_sel = st.multiselect("Supervisor (cód.)", options=supers, default=[], key="fat_sup_sel")
        with colf6:
            vend_sel = st.multiselect("Vendedor (nome)", options=vendedores, default=[], key="fat_vend_sel")

        if estado_sel and "ESTADO" in df.columns:
            df = df[df["ESTADO"].isin(estado_sel)]
        if analista_sel and "ANALISTA" in df.columns:
            df = df[df["ANALISTA"].isin(analista_sel)]
        if sup_sel and "COD_SUP" in df.columns:
            df = df[df["COD_SUP"].isin(sup_sel)]
        if vend_sel and "VENDEDOR" in df.columns:
            df = df[df["VENDEDOR"].isin(vend_sel)]
        ...
        qtd_total = float(df["QTD"].sum())
        clientes_unicos = df["CLIENTE_COD"].nunique() if "CLIENTE_COD" in df.columns else df["CLIENTE_NOME"].nunique()
        skus_unicos = df["SKU_COD"].nunique() if "SKU_COD" in df.columns else df["SKU_NOME"].nunique()

        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Receita", _fmt_money_pt(receita_total))
        c2.metric("Quantidade", _fmt_int_pt(qtd_total))
        c3.metric("Clientes únicos", _fmt_int_pt(clientes_unicos))
        c4.metric("SKUs únicos", _fmt_int_pt(skus_unicos))

        st.markdown("---")

        # ============================
        # Visões: Estado / Analista / Hierarquia / SKU / Cliente
        # ============================
        st.markdown("### 📌 Visões principais")

        colv1, colv2 = st.columns(2)

        with colv1:
            st.subheader("Faturado por Estado")
            if "ESTADO" in df.columns:
                g = df.groupby("ESTADO", as_index=False)["RECEITA"].sum().sort_values("RECEITA", ascending=False).head(15)
                st.bar_chart(g.set_index("ESTADO")["RECEITA"])
                st.dataframe(g, use_container_width=True, hide_index=True)
            else:
                st.info("Coluna de Estado não encontrada.")

        with colv2:
            st.subheader("Faturado por Analista")
            if "ANALISTA" in df.columns:
                g = df.groupby("ANALISTA", as_index=False)["RECEITA"].sum().sort_values("RECEITA", ascending=False)
                st.bar_chart(g.set_index("ANALISTA")["RECEITA"])
                st.dataframe(g, use_container_width=True, hide_index=True)
            else:
        ...
        with colv3:
            st.subheader("Top Hierarquias (Receita)")
            if "HIERARQUIA" in df.columns:
                g = df.groupby("HIERARQUIA", as_index=False)["RECEITA"].sum().sort_values("RECEITA", ascending=False).head(20)
                st.dataframe(g, use_container_width=True, hide_index=True)
            else:
                st.info("Coluna HIERARQUIA não encontrada.")

        with colv4:
            st.subheader("Top SKUs (Receita)")
            sku_col = "SKU_NOME" if "SKU_NOME" in df.columns else ("Nº artigo" if "Nº artigo" in df.columns else None)
            if sku_col is not None:
                g = df.groupby(sku_col, as_index=False)["RECEITA"].sum().sort_values("RECEITA", ascending=False).head(20)
                st.dataframe(g, use_container_width=True, hide_index=True)
            else:
                st.info("Coluna SKU não encontrada.")

        st.markdown("---")

        # ============================
        # SKUs mais vendidos por Analista / Supervisor / Estado
        # ============================
        st.markdown("### 🧠 Insights de “Top SKUs” por dimensão")

        colx1, colx2, colx3 = st.columns(3)

        sku_name_col = "SKU_NOME" if "SKU_NOME" in df.columns else None
        if sku_name_col is None:
            st.info("Sem SKU_NOME na base para montar os insights de Top SKUs por dimensão.")
        else:
            with colx1:
                st.subheader("Top SKUs por Analista")
                if "ANALISTA" in df.columns:
                    dim = st.selectbox("Selecione um Analista", options=sorted(df["ANALISTA"].unique().tolist()), key="sel_analista_topsku")
                    dfx = df[df["ANALISTA"] == dim].copy()
                    g = dfx.groupby(sku_name_col, as_index=False)["RECEITA"].sum().sort_values("RECEITA", ascending=False).head(10)
                    st.dataframe(g, use_container_width=True, hide_index=True)
                else:
                    st.info("Coluna ANALISTA não encontrada.")

            with colx2:
                st.subheader("Top SKUs por Supervisor")
                if "COD_SUP" in df.columns:
                    dim = st.selectbox("Selecione um Supervisor (cód.)", options=sorted(df["COD_SUP"].unique().tolist()), key="sel_sup_topsku")
                    dfx = df[df["COD_SUP"] == dim].copy()
                    g = dfx.groupby(sku_name_col, as_index=False)["RECEITA"].sum().sort_values("RECEITA", ascending=False).head(10)
                    st.dataframe(g, use_container_width=True, hide_index=True)
                else:
            ...
            with colx3:
                st.subheader("Top SKUs por Estado")
                if "ESTADO" in df.columns:
                    dim = st.selectbox("Selecione um Estado", options=sorted(df["ESTADO"].unique().tolist()), key="sel_estado_topsku")
                    dfx = df[df["ESTADO"] == dim].copy()
                    g = dfx.groupby(sku_name_col, as_index=False)["RECEITA"].sum().sort_values("RECEITA", ascending=False).head(10)
                    st.dataframe(g, use_container_width=True, hide_index=True)
                else:
        ...
        # ============================
        # ALERTA 1: Queda de vendas (30d vs 30d anteriores)
        # ============================
        st.markdown("### 🚨 Alertas de queda de vendas (últimos 30 dias vs 30 dias anteriores)")

        # Define janelas com base na data máxima do filtro atual
        max_data = df["DATA_FAT"].max()
        if pd.isna(max_data):
            st.info("Sem datas válidas para calcular queda de vendas.")
        else:
            ...
            df_1 = df[(df["DATA_FAT"] >= start_1) & (df["DATA_FAT"] <= end_1)].copy()
            df_0 = df[(df["DATA_FAT"] >= start_0) & (df["DATA_FAT"] <= end_0)].copy()

            colq1, colq2 = st.columns(2)

            with colq1:
                st.subheader("Vendedores em queda")
                if "COD_VEND" in df.columns:
                    g1 = df_1.groupby(["COD_VEND", "VENDEDOR"], as_index=False)["RECEITA"].sum().rename(columns={"RECEITA": "REC_30D"})
                    g0 = df_0.groupby(["COD_VEND", "VENDEDOR"], as_index=False)["RECEITA"].sum().rename(columns={"RECEITA": "REC_30D_ANT"})
                    gx = g1.merge(g0, on=["COD_VEND", "VENDEDOR"], how="outer").fillna(0.0)
                    gx["VAR_R$"] = gx["REC_30D"] - gx["REC_30D_ANT"]
                    gx["VAR_%"] = gx.apply(lambda r: (r["VAR_R$"] / r["REC_30D_ANT"] * 100.0) if r["REC_30D_ANT"] > 0 else (100.0 if r["REC_30D"] > 0 else 0.0), axis=1)

                    # Queda forte: var% <= -20 e REC_30D_ANT relevante
                    queda = gx[(gx["REC_30D_ANT"] >= 1000) & (gx["VAR_%"] <= -20)].copy()
                    queda = queda.sort_values("VAR_%", ascending=True).head(30)

                    if queda.empty:
                        st.success("Nenhum vendedor com queda relevante (>= R$ 1.000 e <= -20%) nessas janelas.")
                    else:
                        # Formata
                        queda["REC_30D"] = queda["REC_30D"].apply(_fmt_money_pt)
                        queda["REC_30D_ANT"] = queda["REC_30D_ANT"].apply(_fmt_money_pt)
                        queda["VAR_R$"] = queda["VAR_R$"].apply(_fmt_money_pt)
                        queda["VAR_%"] = queda["VAR_%"].map(lambda x: f"{x:.1f}%".replace(".", ","))

                        st.caption(f"Janela atual: {start_1.date()} → {end_1.date()} | Janela anterior: {start_0.date()} → {end_0.date()}")
                        st.dataframe(
                            queda[["COD_VEND", "VENDEDOR", "REC_30D_ANT", "REC_30D", "VAR_R$", "VAR_%"]],
                            use_container_width=True,
                            hide_index=True,
                        )
                else:
                    st.info("Coluna COD_VEND não encontrada.")

            with colq2:
                st.subheader("Supervisores em queda")
                if "COD_SUP" in df.columns:
                    g1 = df_1.groupby("COD_SUP", as_index=False)["RECEITA"].sum().rename(columns={"RECEITA": "REC_30D"})
                    g0 = df_0.groupby("COD_SUP", as_index=False)["RECEITA"].sum().rename(columns={"RECEITA": "REC_30D_ANT"})
                    gx = g1.merge(g0, on="COD_SUP", how="outer").fillna(0.0)
                    gx["VAR_R$"] = gx["REC_30D"] - gx["REC_30D_ANT"]
                    gx["VAR_%"] = gx.apply(lambda r: (r["VAR_R$"] / r["REC_30D_ANT"] * 100.0) if r["REC_30D_ANT"] > 0 else (100.0 if r["REC_30D"] > 0 else 0.0), axis=1)

                    queda = gx[(gx["REC_30D_ANT"] >= 5000) & (gx["VAR_%"] <= -15)].copy()
                    queda = queda.sort_values("VAR_%", ascending=True).head(30)

                    if queda.empty:
                        st.success("Nenhum supervisor com queda relevante (>= R$ 5.000 e <= -15%) nessas janelas.")
                    else:
                        queda["REC_30D"] = queda["REC_30D"].apply(_fmt_money_pt)
                        queda["REC_30D_ANT"] = queda["REC_30D_ANT"].apply(_fmt_money_pt)
                        queda["VAR_R$"] = queda["VAR_R$"].apply(_fmt_money_pt)
                        ...
                        st.caption(f"Janela atual: {start_1.date()} → {end_1.date()} | Janela anterior: {start_0.date()} → {end_0.date()}")
                        st.dataframe(
                            queda[["COD_SUP", "REC_30D_ANT", "REC_30D", "VAR_R$", "VAR_%"]],
                            use_container_width=True,
                            hide_index=True,
                        )
                else:
        ...
        # ============================
        # ALERTA 2: Vendedor sem expansão de mix (poucos SKUs + alta concentração)
        # ============================
        st.markdown("### 🧩 Alertas de “não expandiu mix” (poucos SKUs + 1 SKU domina)")

        if "COD_VEND" not in df.columns or sku_name_col is None:
            st.info("Preciso de COD_VEND e SKU_NOME para calcular mix por vendedor.")
        else:
            # Mix por vendedor no período filtrado
            mix = df.groupby(["COD_VEND", "VENDEDOR"], as_index=False).agg(
                RECEITA=("RECEITA", "sum"),
                QTD=("QTD", "sum"),
                SKUS_UNICOS=(sku_name_col, "nunique"),
                CLIENTES_UNICOS=("CLIENTE_COD" if "CLIENTE_COD" in df.columns else "CLIENTE_NOME", "nunique"),
            )

            # Concentração: % da receita no SKU top do vendedor
            g_sku = df.groupby(["COD_VEND", "VENDEDOR", sku_name_col], as_index=False)["RECEITA"].sum()
            g_sku = g_sku.sort_values(["COD_VEND", "RECEITA"], ascending=[True, False])

            top_sku = g_sku.groupby(["COD_VEND", "VENDEDOR"], as_index=False).first().rename(
                columns={sku_name_col: "SKU_TOP", "RECEITA": "REC_SKU_TOP"}
            )

            mix = mix.merge(top_sku, on=["COD_VEND", "VENDEDOR"], how="left").fillna({"REC_SKU_TOP": 0.0, "SKU_TOP": ""})
            mix["SHARE_TOP_SKU"] = mix.apply(lambda r: (r["REC_SKU_TOP"] / r["RECEITA"] * 100.0) if r["RECEITA"] > 0 else 0.0, axis=1)

            # Regras (ajuste depois como quiser)
            # - SKUs únicos <= 5
            # - Top SKU share >= 55%
            suspeitos = mix[(mix["SKUS_UNICOS"] <= 5) & (mix["SHARE_TOP_SKU"] >= 55)].copy()
            suspeitos = suspeitos.sort_values(["SHARE_TOP_SKU", "RECEITA"], ascending=[False, False]).head(50)

            colm1, colm2 = st.columns([0.55, 0.45])

            with colm1:
                st.subheader("Ranking de vendedores “viciados” (mix baixo + concentração alta)")
                if suspeitos.empty:
                    st.success("Nenhum vendedor bateu as regras de alerta no período filtrado.")
                else:
                    out = suspeitos.copy()
                    out["RECEITA"] = out["RECEITA"].apply(_fmt_money_pt)
                    out["QTD"] = out["QTD"].apply(_fmt_int_pt)
                    out["SHARE_TOP_SKU"] = out["SHARE_TOP_SKU"].map(lambda x: f"{x:.1f}%".replace(".", ","))
                    st.dataframe(
                        out[["COD_VEND", "VENDEDOR", "RECEITA", "QTD", "SKUS_UNICOS", "CLIENTES_UNICOS", "SKU_TOP", "SHARE_TOP_SKU"]],
                        use_container_width=True,
                        hide_index=True,
                    )

            with colm2:
                st.subheader("Detalhe do vendedor")
                vend_options = sorted(df["COD_VEND"].unique().tolist())
                vend_pick = st.selectbox("Escolha o código do vendedor", options=vend_options, key="pick_vend_mix")

                d = df[df["COD_VEND"] == vend_pick].copy()
                nome_v = d["VENDEDOR"].iloc[0] if "VENDEDOR" in d.columns and not d.empty else ""

                st.caption(f"Vendedor: **{nome_v}** | Código: **{vend_pick}**")

                # Top SKUs desse vendedor
                top = d.groupby(sku_name_col, as_index=False)["RECEITA"].sum().sort_values("RECEITA", ascending=False).head(15)
                st.markdown("**Top SKUs (Receita)**")
                st.dataframe(top, use_container_width=True, hide_index=True)

                # Mix por cliente (top clientes)
                cli_col = "CLIENTE_NOME" if "CLIENTE_NOME" in d.columns else None
                if cli_col is not None:
                    topc = d.groupby(cli_col, as_index=False)["RECEITA"].sum().sort_values("RECEITA", ascending=False).head(15)
                    st.markdown("**Top Clientes (Receita)**")
                    st.dataframe(topc, use_container_width=True, hide_index=True)

        st.markdown("---")

        # ============================
        # Drill-down: Vendedor / Cliente / Hierarquia
        # ============================
        st.markdown("### 🔎 Drill-down rápido (pra investigação)")

        colz1, colz2 = st.columns(2)

        with colz1:
            st.subheader("Faturado por Vendedor")
            if "VENDEDOR" in df.columns:
                g = df.groupby("VENDEDOR", as_index=False)["RECEITA"].sum().sort_values("RECEITA", ascending=False).head(30)
                st.dataframe(g, use_container_width=True, hide_index=True)
            else:
                st.info("Coluna VENDEDOR não encontrada.")

        with colz2:
            st.subheader("Faturado por Cliente")
            cli_col = "CLIENTE_NOME" if "CLIENTE_NOME" in df.columns else None
            if cli_col is not None:
                g = df.groupby(cli_col, as_index=False)["RECEITA"].sum().sort_values("RECEITA", ascending=False).head(30)
                st.dataframe(g, use_container_width=True, hide_index=True)
            else:
        ...
        with st.expander("📄 Ver amostra da base filtrada"):

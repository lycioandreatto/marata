    ...
    with tab_login:
        with st.form("login_form"):
            ...
            if st.form_submit_button("Entrar"):
                ...
                else: st.error("Erro na tabela de usuários.")
    ...
    with tab_cadastro:
        with st.form("cad_form"):
            ...
            if st.form_submit_button("Finalizar Cadastro"):
                ...
                else: st.error("Dados inválidos ou senhas não coincidem.")
    st.stop()
...
with st.sidebar:
    ...
    st.markdown("---")
    if st.button("⚠️ APAGAR MINHA AGENDA" if not (is_admin or is_analista or is_diretoria) else "Limpar Agenda Selecionada"):
        # Lógica de limpeza simplificada para brevidade
        pass
...
if menu == "📅 Agendamentos do Dia":
    ...
    if df_agenda is not None and not df_agenda.empty:
        ...
        if not df_dia.empty:
            df_dia["EDITAR"] = False
            cols_v = ['EDITAR', 'DATA', 'SUPERVISOR', 'CLIENTE', 'STATUS', 'AGENDADO POR']
            ...
            if not marcados.empty:
                ...
                with col1: 
                    ...
                    nova_data_reag = None
                    if n_st == "Reagendado":
                        nova_data_reag = st.date_input("Escolha a Nova Data:", datetime.now(fuso_br))
                with col2:
                    n_ju = st.selectbox("Justificativa/Observação:", ju_list)
                ...
                if st.button("💾 ATUALIZAR STATUS"):
                    ...
                    df_agenda.loc[idx, ['STATUS', 'JUSTIFICATIVA']] = [n_st, final_j]
                    if n_st == "Reagendado" and nova_data_reag:
                        df_agenda.loc[idx, 'DATA'] = nova_data_reag.strftime("%d/%m/%Y")
                    conn.update(spreadsheet=url_planilha, worksheet="AGENDA", data=df_agenda.drop(columns=['LINHA'], errors='ignore'))
                    st.cache_data.clear(); st.success("Atualizado!"); time.sleep(1); st.rerun()
        else: st.info(f"Sem visitas para hoje ({hoje_str}).")

# --- PÁGINA: DASHBOARD ---
elif menu == "📊 Dashboard de Controle":
    st.header("📊 Dashboard")
    # Mantendo toda a lógica de filtros e ranking do original
...
# --- PÁGINA: NOVO AGENDAMENTO ---
    ...
    if df_base is not None:
        col_rv_base = next((c for c in df_base.columns if c.upper() == 'REGIÃO DE VENDAS'), 'Região de vendas')
        sup_sel = user_atual if not (is_admin or is_analista or is_diretoria) else st.selectbox("Selecione Supervisor", df_base[col_rv_base].unique())
        ...
        if cliente_sel != "Selecione...":
            qtd_visitas = st.number_input("Quantidade de visitas (Máx 4):", 1, 4, 1)
            with st.form("form_novo_v"):
                ...
                if st.form_submit_button("💾 SALVAR"):
                    ...
                    for i, dt in enumerate(datas_sel):
                        nid = datetime.now(fuso_br).strftime("%Y%m%d%H%M%S") + str(i)
                        novas.append({"ID": nid, "REGISTRO": datetime.now(fuso_br).strftime("%d/%m/%Y %H:%M"), "DATA": dt.strftime("%d/%m/%Y"), "SUPERVISOR": sup_sel, "CÓDIGO CLIENTE": cod_c, "CLIENTE": nom_c, "STATUS": "Planejado", "JUSTIFICATIVA": "-", "AGENDADO POR": user_atual})
...
# --- PÁGINA: VER/EDITAR ---
    ...
    if df_agenda is not None and not df_agenda.empty:
        df_f = df_agenda[df_agenda['SUPERVISOR'] == user_atual].copy() if not (is_admin or is_diretoria) else df_agenda.copy()
        ...
        if not marcados.empty:
            ...
            with col1: 
                ...
                nova_data_reag = None
                if n_st == "Reagendado":
                    nova_data_reag = st.date_input("Escolha a Nova Data:", datetime.now(fuso_br), key="reag_edit")
            with col2: 
                n_ju = st.selectbox("Justificativa:", ju_list)
                mot_outro = st.text_input("Especifique:", key="mot_edit") if n_ju == "OUTRO" else ""

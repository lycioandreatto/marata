...
# --- CONFIGURAÇÃO DE COOKIES (Lembrar Login e Último Acesso) ---
...
def gerar_pdf(df, tipo_relatorio="GERAL"):
    ...
    larguras = {"CLIENTE": 60, "SUPERVISOR": 30, "AGENDADO POR": 30, "DATA": 18, "JUSTIFICATIVA": 50, "REGISTRO": 25, "CIDADE": 40}
    
    especiais = [c for c in larguras.keys() if c in [str(x).upper() for x in cols]]
    ocupado = sum([larguras[c] for c in especiais])
    ...
    for col in cols:
        w = larguras.get(str(col).upper(), largura_padrao)
    ...
    for index, row in df_pdf.iterrows():
        for i, item in enumerate(row):
            ...
            w = larguras.get(col_name, largura_padrao)
...
# --- LÓGICA DE ALERTAS/BADGES ---
user_atual = st.session_state.usuario
ultima_vista = cookies.get("last_view_agenda", "01/01/2020 00:00")
dt_ultima_vista = datetime.strptime(ultima_vista, "%d/%m/%Y %H:%M")

tem_novo_agendamento = False
qtd_nunca_visitados = 0

if df_agenda is not None and not df_agenda.empty:
    df_novos = df_agenda[(df_agenda['SUPERVISOR'] == user_atual) & (df_agenda['AGENDADO POR'] != user_atual)].copy()
    if not df_novos.empty:
        try:
            df_novos['REG_DT'] = pd.to_datetime(df_novos['REGISTRO'], dayfirst=True)
            if not df_novos[df_novos['REG_DT'] > dt_ultima_vista].empty:
                tem_novo_agendamento = True
        except: pass

    if df_base is not None:
        col_rv_base = next((c for c in df_base.columns if c.upper() == 'REGIÃO DE VENDAS'), 'Região de vendas')
        meus_clientes = df_base[df_base[col_rv_base] == user_atual]['Cliente'].unique()
        agendados_realizados = df_agenda[(df_agenda['SUPERVISOR'] == user_atual) & (df_agenda['STATUS'] == "Realizado")]['CÓDIGO CLIENTE'].unique()
        qtd_nunca_visitados = len([c for c in meus_clientes if c not in agendados_realizados])
...
label_display, user_icon, border_color = (
    ("ADMINISTRADOR", "👑", "#FFD700") if is_admin else 
    (f"DIRETORIA {user_atual}", "📈", "#1E90FF") if is_diretoria else
    (f"ANALISTA {user_atual}", "🔬", "#9370DB") if is_analista else
    (f"SUPERVISOR {user_atual}", "👤", "#ff4b4b")
)
...
with st.sidebar:
    ...
    txt_dia = "📅 Agendamentos do Dia" + (" 🔔" if tem_novo_agendamento else "")
    txt_novo = "📋 Novo Agendamento" + (f" ({qtd_nunca_visitados})" if qtd_nunca_visitados > 0 else "")
    
    opcoes_menu = [txt_dia, txt_novo, "🔍 Ver/Editar Minha Agenda"]
...
# --- PÁGINAS ---
if "📅 Agendamentos do Dia" in menu:
    st.header("📅 Agendamentos do Dia")
    cookies["last_view_agenda"] = datetime.now(fuso_br).strftime("%d/%m/%Y %H:%M")
    cookies.save()
    ...
    if df_agenda is not None and not df_agenda.empty:
        ...
        if not df_dia.empty:
            ...
            df_dia["EDITAR"] = False
            if tem_novo_agendamento:
                try:
                    df_dia['REG_DT'] = pd.to_datetime(df_dia['REGISTRO'], dayfirst=True)
                    df_dia['STATUS'] = df_dia.apply(lambda x: "✨ NOVO! " + x['STATUS'] if x['REG_DT'] > dt_ultima_vista and x['AGENDADO POR'] != user_atual else x['STATUS'], axis=1)
                except: pass
            ...
            if not marcados.empty:
                ...
                c1, c2 = st.columns(2)
                with c1: n_st = st.radio("Status:", st_list, index=st_list.index(sel_row['STATUS'].replace("✨ NOVO! ", "")) if sel_row['STATUS'].replace("✨ NOVO! ", "") in st_list else 0)
                with c2: n_ju = st.selectbox("Justificativa:", ju_list)
                if st.button("💾 SALVAR ALTERAÇÃO"):
                    df_agenda.loc[df_agenda['ID'] == sel_row['ID'], ['STATUS', 'JUSTIFICATIVA']] = [n_st, n_ju]
                    conn.update(spreadsheet=url_planilha, worksheet="AGENDA", data=df_agenda.drop(columns=['LINHA'], errors='ignore'))
                    st.toast(f'✅ Visita atualizada para {sel_row["CLIENTE"]}!', icon='✔️')
                    time.sleep(1)
                    st.cache_data.clear()
                    st.rerun()
        else: st.info(f"Sem agendamentos para hoje.")

elif "📋 Novo Agendamento" in menu:
    st.header("📋 Agendar Visita")
    if df_base is not None:
        col_rv_base = next((c for c in df_base.columns if c.upper() == 'REGIÃO DE VENDAS'), 'Região de vendas')
        sup_sel = st.selectbox("Supervisor:", sorted(df_base[col_rv_base].unique())) if (is_admin or is_analista or is_diretoria) else user_atual
        
        clientes_f = df_base[df_base[col_rv_base] == sup_sel]
        agendados = df_agenda[df_agenda['SUPERVISOR'] == sup_sel]['CÓDIGO CLIENTE'].unique()
        clientes_pendentes = clientes_f[~clientes_f['Cliente'].isin(agendados)]
        
        st.write(f"Clientes pendentes de primeiro agendamento: **{len(clientes_pendentes)}**")
        ...
        if lista_c:
            ...
            if cliente_sel != "Selecione...":
                with st.form("f_novo"):
                    d = st.date_input("Data:", datetime.now(fuso_br))
                    if st.form_submit_button("Salvar Agendamento"):
                        cod_c, nom_c = cliente_sel.split(" - ", 1)
                        agora = datetime.now(fuso_br)
                        nova = {"ID": agora.strftime("%Y%m%d%H%M%S"), "REGISTRO": agora.strftime("%d/%m/%Y %H:%M"), "DATA": d.strftime("%d/%m/%Y"), 
                                "ANALISTA": NOME_ANALISTA, "SUPERVISOR": sup_sel, "CÓDIGO CLIENTE": cod_c, "CLIENTE": nom_c, "JUSTIFICATIVA": "-", 
                                "STATUS": "Planejado (X)", "AGENDADO POR": user_atual}
                        df_final_a = pd.concat([df_agenda.drop(columns=['LINHA'], errors='ignore'), pd.DataFrame([nova])], ignore_index=True)
    ...
    if df_agenda is not None:
        ...
        c1.metric("Total Agendado", len(df_agenda))
        c2.metric("Realizados", len(df_agenda[df_agenda['STATUS'] == "Realizado"]))
        c3.metric("Pendentes", len(df_agenda[df_agenda['STATUS'].str.contains("Planejado")]))
        st.write("Relatório Geral de Atividades")

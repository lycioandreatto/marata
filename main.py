...
if not st.session_state.logado:
    ...
    with tab_login:
        with st.form("login_form"):
            u_login, p_login, lembrar = st.text_input("Usuário:").strip().upper(), st.text_input("Senha:", type="password"), st.checkbox("Manter conectado")
        ...
        with st.form("cad_form"):
            ...
            if st.form_submit_button("Finalizar Cadastro"):
                if u_cad and p_cad == p_cad_conf:
                    if u_cad not in df_usuarios['USUARIO'].str.upper().values:
                        novo_user = pd.DataFrame([{"USUARIO": u_cad, "SENHA": p_cad}])
                        conn.update(spreadsheet=url_planilha, worksheet="USUARIOS", data=pd.concat([df_usuarios, novo_user], ignore_index=True))
                        st.success("Cadastro realizado!"); st.cache_data.clear()
                    else: st.error("Usuário já cadastrado.")
                else: st.warning("Verifique os campos.")
    st.stop()

# --- PERFIL DO USUÁRIO ---
user_atual = st.session_state.usuario
is_admin, is_analista, is_diretoria = (user_atual == NOME_ADMIN.upper()), (user_atual == NOME_ANALISTA.upper()), (user_atual == NOME_DIRETORIA.upper())
...
with st.sidebar:
    ...
    if is_admin or is_analista or is_diretoria:
        sup_limpar = st.selectbox("Limpar agenda de:", ["Selecione..."] + sorted(df_agenda['SUPERVISOR'].unique()))
        if sup_limpar != "Selecione..." and st.button(f"⚠️ APAGAR TUDO: {sup_limpar}"):
            conn.update(spreadsheet=url_planilha, worksheet="AGENDA", data=df_agenda[df_agenda['SUPERVISOR'] != sup_limpar].drop(columns=['LINHA'], errors='ignore'))
            st.cache_data.clear(); st.rerun()
    elif st.button(f"⚠️ APAGAR TODA MINHA AGENDA"):
        conn.update(spreadsheet=url_planilha, worksheet="AGENDA", data=df_agenda[df_agenda['SUPERVISOR'] != user_atual].drop(columns=['LINHA'], errors='ignore'))
        st.cache_data.clear(); st.rerun()
    ...
    if df_agenda is not None and not df_agenda.empty:
        ...
        if not df_dia.empty:
            ...
            if not marcados.empty:
                ...
                st_list, ju_list = ["Planejado", "Realizado", "Reagendado"], list(df_just.iloc[:, 0].dropna().unique()) + ["OUTRO"]
                c1, c2 = st.columns(2)
                with c1: n_st = st.radio("Status Atual:", st_list, index=st_list.index(sel_row['STATUS']) if sel_row['STATUS'] in st_list else 0)
...
# --- PÁGINA: DASHBOARD ---
    ...
    if df_base is not None and df_agenda is not None:
        col_ana, col_rv, col_loc = next((c for c in df_base.columns if c.upper() == 'ANALISTA'), 'Analista'), next((c for c in df_base.columns if c.upper() == 'REGIÃO DE VENDAS'), 'Região de vendas'), next((c for c in df_base.columns if c.upper() == 'LOCAL'), 'Local')
        df_bf = df_base.copy()
        if is_admin or is_diretoria:
            ana_sel = st.selectbox("Analista:", ["Todos"] + sorted([str(a) for a in df_base[col_ana].unique() if str(a).strip() and str(a).lower() != 'nan']))
            if ana_sel != "Todos": df_bf = df_bf[df_bf[col_ana] == ana_sel]
        else: df_bf = df_bf[df_bf[col_ana].str.upper() == user_atual]
        
        sup_sel = st.selectbox("Supervisor:", ["Todos"] + sorted([str(s) for s in df_bf[col_rv].unique() if str(s).strip()]))
        if sup_sel != "Todos": df_bf = df_bf[df_bf[col_rv] == sup_sel]
        
        res_b = df_bf.groupby(col_rv).size().reset_index(name='Total')
        res_a = df_agenda[df_agenda['CÓDIGO CLIENTE'].isin(df_bf['Cliente'])].groupby('SUPERVISOR')['CÓDIGO CLIENTE'].nunique().reset_index(name='Agendados')
        ...
        st.dataframe(df_dash[[col_rv, 'Total', 'Agendados', '%']], use_container_width=True, hide_index=True)

# --- PÁGINA: NOVO AGENDAMENTO ---
elif menu == "📋 Novo Agendamento":
    st.header("📋 Agendar Visita")
    col_rv = next((c for c in df_base.columns if c.upper() == 'REGIÃO DE VENDAS'), 'Região de vendas')
    sup_sel = st.selectbox("Supervisor:", sorted(df_base[col_rv].unique())) if is_admin or is_analista or is_diretoria else user_atual
    if sup_sel:
